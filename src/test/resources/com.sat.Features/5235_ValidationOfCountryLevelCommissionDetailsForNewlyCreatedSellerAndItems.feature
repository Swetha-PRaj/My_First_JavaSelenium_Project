...
Feature: TC 5235 Validation of Country level Commission details for newly created seller and items
  ...
  Scenario Outline: TC 5235 Validation of Country level Commission details for newly created seller and items
    ...
    #Then select country settings and enter the commision "<countryComision>"
    ...
    Then validate seller commission with "<sellerCommission>"
    ...
    Then open the "Pending" item and validate the commission with "<countryComision>"
    Then Login to Resalepportal "<url>","<Email>" and delete the created seller

    #svg[@data-appmagic-icon-name="Basel_Cancel"
    Examples: 
      | url                | Email             | Firstname | Lastname | Mobile     | Street Address | Postal code | City     | existingseller    | tab         | invIBANNumber   | validIBANnumber | Brand   | Country | Store             | countryComision | Unsolditem | sellerCommission |
      | resale_portal_url1 | swetha.pr@hcl.com | Swetha    | pr       | 9876543210 | Marathalli     |      554678 | Banglore | swetha.pr@hcl.com | All Sellers | invSbIBANnumber | sbIBANnumber    | WEEKDAY | Sweden  | Drottninggatan 63 |              60 | Pickup     |               60 |
